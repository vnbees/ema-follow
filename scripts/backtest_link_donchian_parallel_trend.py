#!/usr/bin/env python3
"""Donchian parallel-band trend + pullback entry — LINK backtest (5m or 15m).
...
5. Exit long when high touches upper band; exit short when low touches lower band.
"""
...
def run(df: pd.DataFrame) -> tuple[list[dict], dict]:
    ...
    for i in range(n):
        ...
        for t in opens_pos:
            side = t["side"]
            hit = (side == "long" and hi >= up_band) or (side == "short" and lo <= lo_band)
            if hit:
...
def main() -> None:
    ...
    print("Running Donchian parallel-trend backtest...", flush=True)
    trades, st = run(df)
    ...
    lines = [
        f"# Donchian parallel-band trend — {SYMBOL} {INTERVAL} {LOOKBACK_DAYS}d",
        "",
        ...
        "## Tong hop",
        "",
        f"| Trades | Lenh/ngay | WR | PnL | % | PnL/ngay | Long | Short | TP band | EOD | Trend events | Skip | Phi |",
        f"| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        ...
        f"{st['tp_count']} | {st['eod_count']} | {st['trend_events']} | {st['skipped_parallel_entry']} | {st['fee']:.2f} |",
        "",
        "## Tat ca lenh",
        "",
    ...
    for t in trades:
        lines.append(
            ...
            f"{_local(t['entry_ts'])} | {t['entry']:.4f} | {t['tp_band']:.4f} | "
    ...
    out = ROOT / "docs" / f"backtest_LINK_donchian_parallel_trend_{INTERVAL}_{LOOKBACK_DAYS}d.md"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {out}")
    print(
